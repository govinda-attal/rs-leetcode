...
pub fn merge_sorted_list(
    ...
    let l1ref = l1cur.as_ref().unwrap().as_ref();
    let l2ref = l2cur.as_ref().unwrap().as_ref();

    let mut head = if l1ref.val < l2ref.val {
        let val = l1ref.val;
        let node = ListNode::new(val);
        l1cur = l1cur.unwrap().next;
        node
    } else {
        let val = l2ref.val;
        let node = ListNode::new(val);
        l2cur = l2cur.unwrap().next;
        node
    };

    let mut curr = &mut head;

    while l1cur.is_some() && l2cur.is_some() {
        let l1ref = l1cur.as_ref().unwrap().as_ref();
        let l2ref = l2cur.as_ref().unwrap().as_ref();
        let node = if l1ref.val < l2ref.val {
            let val = l1ref.val;
            let node = ListNode::new(val);
            l1cur = l1cur.unwrap().next;
            node
    ...
    if let Some(entry) = l1cur {
        curr.next = Some(entry)
    }

    if let Some(entry) = l2cur {
        curr.next = Some(entry)
    }
